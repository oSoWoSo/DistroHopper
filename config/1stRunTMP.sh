...
replace='\"!\"'
Path=$VMS_DIR
Exec=sh -c 'yad --form --field=\"Release:CB\" \"${releases// /$replace}\" | cut -d\"|\" -f1 | xargs -I{} sh -c \"quickget $get_name {}\"'
Icon=$icon_file
Categories=System;Virtualization;"
...
replace=$replace
Path=$VMS_DIR
Exec=sh -c 'yad --form --separator=\" \" --field=\"Release:CB\" \"${releases// /$replace}\" --field=\"Edition:CB\" \"${editions// /$replace}\" | xargs -I{} sh -c \"quickget $get_name {}\"'
Icon=$icon_file
Categories=System;Virtualization;"
            echo "$desktop_file_content" > "$CONFIG_DIR"/supported/"$vm_desktop".desktop
        fi
